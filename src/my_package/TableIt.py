# Source :https://github.com/CodeForeverAndEver/TableIt
...
def makeRows(rows, cols, largestElementLength, rowLength, matrixToWorkOn, finalTable, color):
    ...
    for i in range(rows):
        ...
        for j in range(cols):
            ...
            if ((color != None) and ( (j == 0 or i == 0) or (j == 0 or i == 1))):
                current_color = [255, 0, 0] if i==0  else color
                # Only add color if it is in the first column or first row
                currentEl = " " + "\033[38;2;" + str(current_color[0]) + ";" + str(current_color[1]) + ";" + str(current_color[2]) + "m" + \
            ...
            if (largestElementLength != len(matrixToWorkOn[i][j])):
                ...
                if (color != None):
                    if (i == 0):
                        col_range = range(0, cols-1)
                        titel_color = [255,0,0]
                        seperator = " " if (i == 0 and j in col_range ) else "|"
                        currentEl = currentEl + " " * (largestElementLength - (len(currentEl) - len(
                            "\033[38;2;" + str(titel_color[0]) + ";" + str(titel_color[1]) + ";" + str(
                                titel_color[2]) + "m" + "\033[0m")) + 2) + seperator
                    elif (j == 0 or i == 1):
                        currentEl = currentEl + " " * (largestElementLength - (len(currentEl) - len(
...
def createRowUnderFields(largestElementLength, cols, finalTable):
    ...
    finalTable.insert(4, rowUnderFields)
...
def printTable(matrix, title, useFieldNames=False, color=None):
    ...
    title_row = ["  "] * len(matrix[0])
    title_row[location] = title
    matrix.insert(0, title_row)
